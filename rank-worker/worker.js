...
async function handleSync(req, env, uid, token, cors) {
  ...
  //     (읽기 실패 시엔 wordIds=null → 판정 보류하고 종전처럼 카운트: 가용성 우선. 조작에는
  //      유리하지 않다 — 읽기 실패는 곧 doneByDay 도 비어 wk=0 이 되기 때문.)
...
export const _internals = { validWeek, clampInt, sortBoard, normIds, isoAddDays, streakCompute, countWeekDone, countDayDone, countAllDone, validId, weekMondayKST, parseDoneByDay, parseWordIds, kstToday, pruneDays, streakFromDays, isMasterUser, sameMeta };
