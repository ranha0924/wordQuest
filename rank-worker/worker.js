...
//   he=sha256(SALT|'en'|normEn(w.w)), hk=sha256(SALT|'ko'|normKo(w.m)). 산출물엔 해시만(원문·솔트 없음).
//   ★단어파일이 바뀌면 ANSWER_SALT 로 재실행→두 트윈 갱신→재배포. 비어 있으면 팩 검증 0건(개인 경로만).
...
async function handleSync(req, env, uid, token, cors) {
  ...
  //   ver 이 비면 attWk 그대로(회귀 0). ATT_RETIRED=true 면 ver 만(콘솔 doneByDay 한 줄 위조 무력화 · id-에코 잔여는 이상탐지+상한).
