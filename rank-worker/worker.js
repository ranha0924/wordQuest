...
async function handleSync(req, env, uid, token, cors) {
  ...
  return json({ wk: wk, streak: streak, cid: cid || null, ceiling: countAllDone(doneByDay, wordIds), degraded: degraded }, 200, cors);
}
...
async function handleBoard(env, uid, token, url, cors) {
  ...
  try {
    ...
    if (pst.wordIds !== null || Object.keys(pst.doneByDay).length) {
      ...
      const sToday = kstToday(Date.now());
      const wkLive = countWeekDone(dbd, week, sToday, wordIds);        // 유효 단어만
      let st = null;
...
export const _internals = { validWeek, clampInt, sortBoard, normIds, isoAddDays, streakCompute, countWeekDone, countDayDone, countAllDone, validId, weekMondayKST, parseDoneByDay, parseWordIds, kstToday, pruneDays, streakFromDays, isMasterUser, boardWriteDue };
