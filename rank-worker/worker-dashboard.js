...
async function handleFetch(req) {
  ...
  var env = {
    ...
    PERSONAL_WEEK_CAP: (typeof PERSONAL_WEEK_CAP !== 'undefined') ? PERSONAL_WEEK_CAP : '',  // r18: 개인 레인 캡(0=차단) — SW형은 이 패스스루가 없으면 env 미전달로 킬스위치 무동작
  ...
  const cors = {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Firebase-AppCheck',   // r19: App Check 헤더 허용(치명 수정 — 없으면 토큰 동봉 브라우저의 프리플라이트가 거부돼 정상 앱만 실패)
...
//        통과하는 역전 상태였다(이번 '외부 침해' 관측의 유력 설명). 허용 헤더 추가는 순수 완화 = 회귀 0.
//        ② verifyAppCheck 페일클로즈드 — APPCHECK_ENFORCE=true 인데 PROJECT_NUMBER 누락이면 misconfig 거부
//        (명시적 강제 스위치의 조용한 무력화 제거). ③ /sync 응답에 acw(App Check 미검증 사유) — enforce 켜기
//        전에 토큰 도달·검증을 확인하는 모니터링 필드(검증 성공 시 필드 없음). 순서 런북: docs/appcheck-setup.md.
...
async function handleSync(req, env, uid, token, cors, ac) {
  ...
  return json({ wk: wkC, attWk: wk, streak: streak, cid: cid || null, ceiling: countAllDone(doneByDay, wordIds), bound: bound, dailyCap: dailyCap, weekCap: weekCap, degraded: degraded, attDegraded: att.degraded, acw: (ac && (ac.degraded || (ac.skipped ? 'skipped' : null))) || undefined }, 200, cors);  // acw: 미검증 사유(모니터링) — 검증 성공 시 undefined = JSON 에서 필드 탈락
}
...
//   r19 페일클로즈드: enforce=true 인데 PROJECT_NUMBER 미설정이면 misconfig 로 거부 — 명시적 강제 스위치가
//   조용히 무력화되던 구멍 제거. 비강제 시 PROJECT_NUMBER 미설정은 기존대로 스킵(회귀 0).
