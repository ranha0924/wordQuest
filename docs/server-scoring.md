...
## 8. 이상 탐지 + 교사 플래그 (r14) — '탐지+실격'이 교실의 실용적 완전차단

완전 차단이 C1 로 불가하므로, **의심스러운 활동을 서버가 감지해 선생님 대시보드에 ⚠️ 표시**한다
(자동 실격이 아니라 **선생님이 확인·판단**). 교실 맥락에선 이 '탐지 + 교사 실격'이 사실상의 완전차단이다.

- **감지 신호**(`/quiz/submit`): ① **비인간적 제출 속도** — 클라 제출 타임스탬프(`t`)의 중앙 간격이
  `FLAG_FAST_MS`(기본 500ms) 미만(답 ≥5개일 때). ② **일 버스트 상한 초과 시도**(`capped`).
  누적 → `flag:{uid} = {f:빠른제출, b:상한초과, at:최근날}`(21일 롤링).
- **표시**: `/teacher` 가 학생별 flag 반환 → 대시보드 학생 카드 이름 옆 **⚠️**(툴팁에 사유·횟수).
- **정직한 한계**: 클라 `t` 는 위조 가능이라 **느린(사람 속도로 sleep 하는) 스크립트는 못 잡는다** —
  '게으른 자동화'와 '상한 어택'을 잡는 best-effort 신호다. 오탐도 가능(진짜 빠른 학생) → 선생님이 확인.
  더 강한 탐지가 필요하면 서버측 세션 타이밍(quizStart↔submit 분리)·행동 통계로 확장 가능(백로그).
