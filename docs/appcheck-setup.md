...
즉 **App Check 만으로 위조가 100% 차단되지는 않는다.** 위조를 실제로 죽이는 것은 **서버 세션 채점**
(`/quiz/start`·`/quiz/submit` — 랭킹 크레딧을 '서버가 발급한 세션에서 실제로 푼 것'으로만 인정)과
**버스트/주간 상한**이다. App Check 는 그 위에 **"앱 밖 대량 자동화"를 걸러 내는 방벽**을 한 겹 더
얹는 역할이다. (설계 근거는 `docs/server-scoring.md` 참고.)
...
### 4-b) 워커 `/quiz/*` 강제
Firestore 강제와 **별개**로, 랭킹 워커의 세션 엔드포인트도 App Check 를 강제하려면 워커 env
`APPCHECK_ENFORCE=true` 로 바꾼다(재배포 불필요, 변수만 변경 → 즉시 적용). 이 스위치·롤아웃 순서는
`docs/server-scoring.md` 에서 다룬다.
