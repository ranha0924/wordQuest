...
2. **차단 경로**: 앱 밖 호출이 403 으로 막히는지 — 로그인 상태 앱의 DevTools Network 에서 `/sync` 요청의
   ...
   # → {"v":"r19","error":"appcheck","reason":"no_token"} = 앱 밖 위조 경로가 실제로 차단되고 있다는 뜻
...
유료 Claude 호출·`DAILY_CAP` 을 태우는 공격 차단). **코드 배포와 env 강제의 시점이 다르다**:
1. **코드(o2) 배포는 즉시**(체크리스트 ① — rank-worker r19 와 동시) — 도먼트(무위험)이고,
   ★o2 의 CORS 가 App Check 헤더를 허용해야 **v125 클라의 AI OCR 이 끊기지 않는다**
   (구 o1 워커 + v125 조합이면 프리플라이트 실패로 AI OCR 이 조용히 기기 내 OCR 로 강등 —
   구현검수 지적). 배포 확인: 워커 주소 열어 `{"v":"o2","error":"method_not_allowed"}`.
2. **env 강제는 앱 v125 배포·전파 확인 후** — v125 부터 OCR 호출에 App Check 헤더를 동봉하고,
   403 `appcheck` 를 받으면 **기기 내 무료 OCR(Tesseract)로 자동 강등**한다. ⚠️ v124 이하 구캐시
   클라는 403 시 폴백 없이 "인식 서버 오류(appcheck)" 안내만 보므로(수동 입력은 가능), 전파 전에
   켜면 AI 인식이 그만큼 불편해진다 — 급하지 않으니 전파 후에 켤 것.
...
- [ ] **① rank-worker 트윈 r19 + ocr-worker o2 재배포**(`v:"r19"`·`v:"o2"` 확인) — 둘 다 도먼트(무위험). r19 는 CORS 수정으로 정상 학생 `/sync`·`/quiz` 즉시 복구, o2 는 v125 클라의 AI OCR 프리플라이트 통과에 필요(4-c ①).
...
- [ ] **⑥ (선택) ocr-worker env `PROJECT_NUMBER`+`APPCHECK_ENFORCE=true`**(4-c — v125 전파 후. 코드 o2 는 ①에서 이미 배포됨).
