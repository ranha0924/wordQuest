...
- **영입소** 등록 경로: (1) 붙여넣기(`abandon - 버리다` 형식, `parseText()`), (2) 샘플 20개 버튼, (3) **"고1 필수 어휘 팩 담기"**(`packbtn`, 추천·골드 실선) = 주제별 큐레이션 224 전체(하루 25개 staggered), (3b) **"주제별로 골라 담기"**(`packthemes-toggle`→14주제 `.fchip` 칩, `renderPackThemes`/`seedTheme`) = 고른 주제만 **즉시 담기(집중 학습)**·담긴 주제는 ✓ 표시, (4) **"고빈도 영단어(범용) 담기"**(`bankbtn`, 이전 "교과서 고빈도"에서 **정직하게 개칭**) = 범용 은행(하루 25개). 모두 이미 담긴 단어 제외.
