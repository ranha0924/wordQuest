...
> - **배포 ★재정정(2026-07-20)**: **학생 실사용 주소는 Vercel `https://word-quest-fywr.vercel.app/`** 이다(월요일 랭킹 사고 조사 중 확인 — 아래 07-15 정정의 "Pages가 배포"는 실사용과 불일치했고, 이 착오로 reCAPTCHA 키 도메인·API 키 리퍼러가 `ranha0924.github.io`만 허용해 **App Check 어테스트가 전교생 단위로 실패**했다). GitHub Pages(`ranha0924.github.io/wordQuest`)도 main push 시 병행 배포됨. **도메인 의존 허용목록(reCAPTCHA 키 도메인·GCP API 키 리퍼러·Firebase Auth 승인 도메인)에는 반드시 `word-quest-fywr.vercel.app` 포함**(Pages 병기 가능). 옛 Vercel `word-quest-self-one.vercel.app` 은 폐기 대상(허용목록 추가 금지). 주소 단일화는 추후 과제.
> - (구) **배포**: Vercel 아님 → **GitHub Pages**(`.github/workflows/deploy-pages.yml`, main push 시 자동). 배포 URL은 `https://ranha0924.github.io/wordQuest/`.
...
| 커밋 | 내용 |
|---|---|
| (v126 / 워커 r20) | **월요일 랭킹 0 사고 진단 + 소급 보정 레버 (CLAUDE.md 4단계: 플랜 독립검수 90 · 구현 독립검수 92 통과 — 경미 지적 반영: BF_MAX 120→60(서브리퀘스트 한도 여유)·감사 로그 bf: 누적(덮어쓰기 제거)·클라 부분처리 안내)** — 증상: 주간 리셋일(월) 대시보드에 "오늘 114·24단어" 학생들이 랭킹 0/미등재, 선생님 본인 실시간 플레이도 미반영. **진단(2중 원인)**: ① 오전 — 구워커가 라이브라 v12x 클라와 `/quiz` 프로토콜 불일치(questions/tids 부재 → sid 미설정 → 제출 0) = 채점 레인 침묵. att 는 r19 배포 후 첫 sync 에 당일치 소급 관측되어 ✓ 는 정상 → **"✓ 있는데 랭킹 0"**(랭킹은 r17부터 실시간 채점분만이라 소급 불가). 리셋이 고장을 만든 게 아니라 지난주 잔존 점수가 가리던 고장을 드러냄. ② 11:10~11:18 KST — 콘솔 로그로 확정: `APPCHECK_ENFORCE=true` 켠 직후부터 `/sync`·`/quiz/start` 전원 403. 근본은 **사이트 전체 `appCheck/recaptcha-error`**(토큰 발급 자체 실패 — 키 종류/도메인/등록 문제 추정)인데 acw 모니터링 정착 전에 enforce 를 켬 → **즉시 롤백(`APPCHECK_ENFORCE=false`, 완료)**. **r20**: `POST /teacher/backfill?class&day&after`(마스터 전용) — 학생별 `doneByDay[day]` 실제 id 를 `ver_mc`/`ver_ps` 에 `verAddDay` 적립(주간 유니크라 재실행·이후 실채점과 **이중 카운트 원천 불가**) + 보드 즉시 갱신(mem 이름 시드). 통제: words fail-closed·personal 뜻 선재+킬스위치·일 버스트를 기존 원장과 **합산** 컷·8회/10분·감사 `bf:` 키·120명/호출+`next` 커서. `rankWk` 산식을 `rankScore`(순수)로 분리(동작 불변·백필이 재사용해 서브리퀘스트 절약). **v126**: `Cloud.teacherBackfill` + 대시보드 "🛠 오늘 푼 단어 랭킹 반영(보정)" 버튼(마스터에게만 노출·커서 자동 이어하기·결과 요약). 검증: r20 단위·통합 **41/41**(멱등·버스트 합산·킬스위치·no_words 스킵·403/400/429·보드 시드·verAddDay dedup·next 커서 이어하기·감사 누적) · `node --check` 워커 트윈+cloud.js · 트윈 파리티(변경 4함수 **바이트 동일**) · index 메인블록 문법 OK. 캐시 sw.js/BUILD **v126**. `firestore.rules` 무변경(마스터 private read 는 기존 131행 활용). **⚠️ 적용(사용자)**: ① reCAPTCHA 키 점검(재-enforce 는 그 후) ② r20 트윈 붙여넣기(`v:"r20"` 확인) ③ v126 main 병합(Pages) ④ 마스터로 대시보드 → 반 선택 → 보정 버튼. |

| (v127) | **reCAPTCHA 사이트키 재발급·교체** — 구키(`6Lcl71kt…`)는 유형 v3·도메인 정상임에도 전 브라우저(시크릿 창 포함)에서 `appCheck/recaptcha-error`(어테스트 실패). api.js 200·anchor 무에러라 키 내부 상태 문제로 판단 → 신키 발급·`APPCHECK_SITE_KEY` 교체(+`firebase-config.js?v=127` 캐시버스터·sw/BUILD v127). 비밀키는 Firebase App Check 콘솔 등록(사용자·저장소 밖). 자명한 단일값 교체라 4단계 사이클 생략. 검증: 배포 후 시크릿 창 콘솔 무에러 + `/sync` 응답 acw 부재 확인(사용자). enforce 재개는 2~3일 모니터링 후. |
| (최종 결론) | **★어테스트 실패의 진짜 원인 확정(같은 날 저녁) — "키의 도메인 목록에 실사용 주소가 실제로 등록되어 있지 않았음"** (구키: `ranha0924.github.io`만 · 학생 실주소는 `word-quest-fywr.vercel.app` / 신키: 추가 시도가 저장 안 됨 — 다른 키 편집·칩 미확정·오타류). 신키 재발급 자체는 불필요했으나 무해. **진단을 교란한 핵심 함정**: reCAPTCHA v3의 원본 execute 경로는 **도메인이 틀려도 토큰을 발급**하며 배지에만 "사이트 소유자 오류: 도메인" 경고를 띄움 → 원본 방식 테스트(`appcheck-diag.html` A모드)가 거짓 합격. 반면 **Firebase SDK(ReCaptchaV3Provider)의 위젯 렌더 경로는 도메인을 엄격 검증**해 error-callback → `appCheck/recaptcha-error` 즉시 실패 — 그래서 "A 합격·앱만 실패" 패턴이 나옴. **판정은 반드시 `appcheck-diag.html?sdk=1`(B모드 — 앱과 동일 SDK 경로)로 할 것.** 도메인을 정확히 등록하자 B모드·앱 모두 정상화(recaptcha-error 소멸). **enforce 재개 체크리스트**: ① 학생 기기 2~3대에서 `?sdk=1` ✅ 확인 ② 2~3일 콘솔·acw 무이상 ③ 워커 env `APPCHECK_ENFORCE=true` → 30분 관찰(이상 시 false 즉시 롤백·재배포 불요). |

### 이번 세션(2026-07-19) 추가

| 커밋 | 내용 |
|---|---|
| (v125 / 워커 r19 / OCR o2) | **App Check 실효화 — "또 외부에서 뚫림" 대응, enforce 가능 상태 확보 (CLAUDE.md 4단계: 플랜 독립검수 91 · 구현 독립검수 92 통과)** — 조사 결과 **워커 3종 모두 CORS `Access-Control-Allow-Headers` 에 `X-Firebase-AppCheck` 누락(치명)**: 클라 v121부터 이 헤더로 App Check 토큰을 동봉하므로, App Check 가 정상 초기화된 브라우저일수록 **프리플라이트 거부로 `/sync`·`/quiz/*` 가 전부 조용히 실패**(로컬 폴백 = 랭킹 크레딧·att·streak 누락)하고 **CORS 를 무시하는 앱 밖 스크립트만 통과**하는 역전 상태였다(외부 위조가 보드 점령+정상 학생 미등재 관측과 정합 · 이대로 enforce 켰으면 정상 학생 전원 차단 사고). **수정(r19 트윈)**: ① CORS 허용 헤더 추가(순수 완화·회귀 0 — 이것만으로 정상 학생 /sync·/quiz 즉시 복구), ② `verifyAppCheck` 페일클로즈드(`APPCHECK_ENFORCE=true`+`PROJECT_NUMBER` 누락 = misconfig 전면 거부 — 강제 스위치가 조용히 무력화되던 구멍 제거), ③ `/sync` 응답 `acw`(미검증 사유, 성공 시 필드 없음) — enforce 켜기 **전** 토큰 도달·검증을 DevTools Network 로 확인하는 모니터링 필드. **OCR 워커 o2**: 동일 게이트 자립 이식(도먼트 — env 미설정 시 100% 무회귀)+CORS+`v` 리비전 응답 도입, 앱 밖 유료 Claude 호출(비용·DAILY_CAP 소진) 차단 준비. **클라 v125**: `Cloud.getAppCheckToken` 노출(NOOP 스텁 포함), OCR 호출 토큰 동봉+**403 appcheck → 기기 내 무료 OCR 강등**(플랜검수 P1 반영 — 구캐시·광고차단기 학생 기능 보존), `/board`·`/teacher` 선동봉(무해·향후 게이트 대비), acw 콘솔 경고(세션 1회). 검증: verifyAppCheck 단위테스트 **31/31**(rank r19·ocr o2 이식본 동시 — 페일클로즈드/무토큰/변조/만료 60s 유예/aud/iss/no_key/비강제 degraded + acw 직렬화 탈락) · `node --check` 전 파일 · 트윈 파리티 diff(기존 let/var·export 편차 외 0). **⚠️ enforce 스위치는 전부 콘솔 잔여(사용자 작업)**: ① 트윈 r19 + OCR o2 붙여넣기 배포(`v:"r19"`·`v:"o2"` 확인 — 최우선·둘 다 도먼트, r19 만으로 정상 학생 복구·o2 는 v125 AI OCR 프리플라이트 통과에 필요[구현검수 지적 반영: ⑥→① 이동]) → ② 워커 env `PROJECT_NUMBER=363343730753` → ③ v125 main 병합(Pages 자동배포) → ④ 워커 env `APPCHECK_ENFORCE=true`(앱 밖 /sync·/quiz 위조 차단 = 이번 침해 경로) → ⑤ Firebase 콘솔 App Check→Firestore 강제 → ⑥ (선택) OCR 워커 env `PROJECT_NUMBER`+`APPCHECK_ENFORCE=true`(v125 전파 후). 각 단계 검증·롤백: `docs/appcheck-setup.md` 체크리스트(r19). |
| (v124 / 워커 r18) | **타이핑·소탕·개인단어 학습도 랭킹 반영 — 검증 레인 확장 (CLAUDE.md 4단계: 플랜 독립검수 84→94 통과)** — 배경: r17/v123 은 배틀 **e2k(뜻 고르기)+팩**만 랭킹에 넣어(복습 단계는 `RANK_E2K_P=0.25` 확률로만 e2k) 타이핑(k2e·listen·cloze)·k2e4·무리 짝맞추기·개인단어가 전부 미반영 → **성숙 단어 많은 헤비 학습자일수록 랭킹이 실학습량을 크게 밑돎**(사용자 요구로 반영 확대). **설계**: ① **typed 레인** — 위 모드들의 '정답'을 클라가 `{id, typed:답}` 으로 제출, 서버가 `typed(trim·lower)===id` 대조 후 레인별 원장(팩→`ver_mc`·개인→`ver_ps`)에 적립. `/quiz/start` 응답에 `tids`(세션 id 목록) 신설 — **tids 없으면(구워커 r17) 클라가 typed 를 아예 안 보냄**(구워커는 typed 를 '오답 pick'으로 집계해 amc·lowacc 오염 → 무회귀 게이트). **typed 는 amc(주 3회 시도상한) 미소비·lowacc 분모 미산입**(답이 결정적이라 봇 억제 효용 0, 오답 선착 잠금·타이핑 약체 오탐 부작용만 — 플랜검수 중대1·2 반영), 클라는 **정답만 push**(오답은 서버 카운터 무접촉). ② **개인단어 레인** — `/quiz/start` 가 학생 state 문서의 **본인 뜻**으로 4지선다 생성(`parseWordMeanings`·추가 read 0), 크레딧은 **신규 원장 `ver_ps`**(r13 유물 `ver_pers` 미부활). 점수 = `min(ver_mc, MC_WEEK_CAP=2000) + min(ver_ps−ver_mc 겹침, PERSONAL_WEEK_CAP=150)` — 차집합은 주중 배포팩 편입 이중카운트 방지. **`PERSONAL_WEEK_CAP=0` = 개인 레인 킬스위치**(전용 파서 `cap0Of` — `capOf` 는 하한 1이라 0 불가 · **SW형 트윈 env 셔임에 패스스루 복원**, r16 때 삭제됐던 것 — 플랜검수 중대3). ③ 뜻 없는 팩 id·MC 못 만드는 개인 id 는 **typed 전용**(`c:null`)으로 세션 포함, 세션 성립 게이트를 `questions`→`sess.q` 기준으로 교체(typed 전용 세션 400 방지). ④ 일 버스트(`PACK_DAILY_BURST=350`)는 **두 레인 합산**, 원장 put 은 **변경 시에만**(KV 쓰기 절약). ⑤ 클라: `wqTypedPush`(tset 게이트) — `submitSpell` 정답, `answer` k2e4 정답, `matchEval` 짝맞추기 정답 3지점 push. e2k pick(오답 포함 제출=추측 통계)은 무변경. **추가 소탕은 같은 배틀 루프라 자동 포함 · 필살기 소탕(무응답 자동처치)·오프라인은 계속 미반영**. ⑥ NEWS_VER 20260719a 인앱 고지. **정직한 한계(명시)**: typed 의 답은 요청의 id 그 자체 → **스크립터에겐 id-echo 등가 재개방**(C1 공개 단어은행 탓에 e2k MC 도 인페이지 ~100%인 것과 같은 평면). 실효 방어 = **App Check enforce(사용자 콘솔 작업 아직 대기)+레이트리밋+일 버스트+주캡+세션 흐름 강제**('콘솔 한 줄' 위조는 계속 불가). 검증: `node --check` 트윈+index 4블록 · 세션흐름 기능테스트 **32/32**(typed 정/오답·amc 면제·lowacc 분리·개인 캡/킬스위치·typed 전용 세션·버스트 합산·차집합·구클라 pick 호환·리플레이/seen 가드) · 구클라 베이스라인 6/6 · **트윈 파리티(공유구간 바이트 동일+셔임 키 대조)** · **Playwright 브라우저 스모크 11/11**(k2e 정/오답·k2e4·e2k pick 불변·무리 조우 4쌍·no-tids 무회귀 게이트). 캐시 sw.js v123→**v124**·BUILD v124. `firestore.rules` 무변경. **⚠️ 적용: 트윈 재배포(`v:"r18"` 확인) + 앱(Pages) 배포. 개인 레인 즉시 차단은 워커 env `PERSONAL_WEEK_CAP=0`**(재배포 불필요). 상세: `rank-worker/README.md` r18 절. |

### 이번 세션(2026-07-18) 추가

| 커밋 | 내용 |
|---|---|
| (v122 / 워커 r17) | **랭킹 위조 근본 수정 2단계 — 서버 출제 MC·index 채점 (id-echo 제거 · CLAUDE.md 4단계: 플랜 독립검수 90 통과)** — 1단계(App Check)로도 '진짜 페이지 개발자도구'에서 답=단어 id 를 그대로 제출하는 **id-echo** 는 남았다(정답이 공개파일). 2단계는 **채점에서 '정답 텍스트'를 완전 제거**: `/quiz/start` 가 서버에서 **뜻 4지선다**를 만들고 **정답 번호(index)는 세션(KV)에만** 저장, 클라엔 보기 4개만 준다. `/quiz/submit` 은 **'고른 번호'만** 받아 `pick===정답index` 로 채점 → 답을 되던질 곳이 없어 **id-echo 원천 무효**. **랭킹 단일화**: 점수를 **서버 MC 검증분(`ver_mc`)만**(`rankWk`)으로 — att·옛 hash(`ver_pack`) 경로 **은퇴**(3 writer: handleSync·updateQuizBoard·handleBoard 내행 전부 `rankWk`). att 는 교사 대시보드 출석·연속(streak) 표시용으로만 유지. **★랭킹 리셋**: 배포 즉시 랭킹은 'MC 퀴즈 푼 만큼'으로 재정비(타이핑·개인·스캔단어 랭킹 미반영). **정직한 한계(★중요)**: 뜻이 오프라인용 공개파일이라 서버가 4보기(정답 뜻 포함)를 내리면 **'공개 뜻을 긁어 번호를 맞추는 인페이지 스크립터(T3)'는 여전히 통과**(~100% 정답이라 저정답률 탐지도 회피, 전체 은행까지 위조 가능). 2단계가 실제로 하는 것: ① id-echo·복붙 원라이너 사망, ② 위조에 '뜻 스크레이핑' 노력 강제, ③ 저정답률 탐지(블라인드 25%), ④ App Check enforce(앱 밖 차단)와 결합. **완전차단 아님.** **구성**: 워커 트윈 r17(handleQuizStart MC출제·handleQuizSubmit index채점·`rankWk`·재시도상한 `amc:uid:week`·저정답률 플래그·`getClassPackMc` 평문·`mcBuild`/`mcPool`·`packAnswerOk`/`combinedWk` 폐기), `scripts/build-pack-answers.mjs`→**`PACK_MC` 평문 번들**(1131 id·양 트윈 주입·SALT 불요), cloud.js(quizStart/quizSubmit 범용 패스스루라 무변경), index.html(**'랭킹 퀴즈' 모달 신설**·배틀 답텍스트 버퍼(wqQuizBuffer/Flush) 제거·배틀은 학습 전용). 재시도 상한은 **제출당 id 1회 채점(index-나열 공격 차단) + 주간 상한 `MC_ATTEMPTS_PER_ID=3`**(블라인드 ≤1−0.75³≈58%). 신규 env `MC_WEEK_CAP`(2000)·`MC_ATTEMPTS_PER_ID`(3)·`FLAG_LOWACC_MIN`(5)·`FLAG_LOWACC_RATE`(0.5). 검증: 순수유닛 **25/25**(mcBuild 정답포함·중복/부분겹침제거·후보<3 null·정답index정확 / rankWk mc-only·cap / handleQuizSubmit index채점·**답텍스트무시**·재시도상한 / handleQuizStart **정답 index 미유출**·세션 저장), 트윈 파리티(8함수 **바이트 동일**), `node --check` 전 파일, 클라 메인블록 문법 OK. 캐시 sw.js/index **v122**. `firestore.rules` 무변경. **⚠️ 적용(사용자)**: **(1) 번들은 이미 빌드·커밋됨** → **트윈 재배포(`v:"r17"` 확인)만** 하면 됨(단어 파일 바꾸면 `node scripts/build-pack-answers.mjs` 재실행). (2) env `MC_*`(선택·기본 OK). (3) **App Check enforce 는 1단계에서 진행 중** — 2단계도 앱-밖 backstop 은 App Check 뿐(인페이지 T3 은 미차단). (4) **학생 고지**: 앱 랭킹 모달에 "랭킹은 이제 '랭킹 퀴즈'로 점수를 얻어요" 안내 내장. 상세: `docs/server-scoring.md`(r17 배너)·`rank-worker/README.md`(§r17). |
| (v121 / 워커 r16) | **랭킹 위조 재침해 대응 — "App Check 우선(1단계)" (사용자 선택 범위 · CLAUDE.md 4단계: 플랜 독립검수 90 통과)** — 재침해 진단: 표시 랭킹 `wk`(`combinedWk=max(attWk,verWk)`)로 위조가 드는 경로 ① 팩 **id-echo**(답=id 제출 시 `packAnswerOk` 통과 — id=영어단어이고 단어은행이 공개파일), ② `/quiz/submit` **무채점 개인단어**(personal 무조건 적립 ≤500/주·정답 0으로도), ③ 옛 att 가 `max()` 로 합류. + App Check off(`APPCHECK_SITE_KEY=""`·워커 `APPCHECK_ENFORCE≠true`)라 앱 밖 스크립트가 자기 로그인 토큰만으로 워커를 직접 호출. **정직한 한계**: 단어은행이 오프라인용으로 공개돼 모든 문제 정답이 공개도출 가능 → **인페이지 콘솔 위조는 완전차단 불가**(App Check enforce 가 최대 완화나 인페이지는 유효토큰 보유). **사용자 선택 = 랭킹 리셋·서버-MC 재설계 없이 "App Check 우선"**: (A) `/sync` 에도 App Check 게이트 추가(`/quiz/*` 와 동일 패턴 · **enforce 켜기 전엔 도먼트**=회귀0), (B) **무채점 개인단어 크레딧 제거**(handleQuizSubmit personal 분기 → 미적립·`ver_pers` 미기록), (C) `combinedWk`·제출 점수식에서 personal 항·`ver_pers` 읽기 삭제 → 랭킹=`max(attWk, packWk)`, (D) cloud.js `rankSync` 가 App Check 토큰 동봉(설정 시). **무회귀 논거(정직)**: 개인단어는 att(doneByDay)로 이미 집계되므로 personalWk 제거는 **att 미포화 학생 표시값 불변**, 단 **att 상한(주 300·일 60) 초과 정직 헤비 개인학습자는 상한 초과분만 감소**(개인은 채점불가라 정직분과 위조분을 서버가 분리 못 함 = 의도된 트레이드오프 · 완화 노브: `RANK_WEEK_CAP`/`RANK_DAILY_CAP` env 상향 시 att 관측분 보존, 단 att-시딩 천장도 동반 상향). **남는 것(정직)**: 인페이지 팩 id-echo·att 시딩 — App Check enforce(앱 밖 차단)+상한+이상탐지(교사 ⚠️)로 억제, 근본제거는 **2단계 서버-MC(서버 출제 뜻 4지선다·index 채점) 로 유보**. 검증: `node --check`(양 워커·cloud.js), **트윈 파리티**(combinedWk·handleQuizSubmit 두 워커 **바이트 동일**), 순수유닛(도먼트 게이트·개인 미적립·경계 매트릭스: att 미포화 불변+att 포화 헤비 하락). 캐시 sw.js v120→**v121**·index BUILD v120→v121. `firestore.rules` **무변경**. **⚠️ 적용**: 트윈 재배포(`v:"r16"` 확인) + 앱(Pages v121) → **(B)(C) 개인 곁길 즉시 봉쇄**. **★App Check 실효화는 사용자 콘솔 작업 필수**(config `APPCHECK_SITE_KEY` 기입 + 워커 `APPCHECK_ENFORCE=true`+`PROJECT_NUMBER` + 권장 Firestore App Check enforce) — **이 작업 전엔 '앱 밖 차단' 미동작**(그럼 순이득=개인 곁길 봉쇄뿐). 상세: `docs/appcheck-setup.md`(§0 r16 자세·§4-b `/sync` blast radius·즉시 롤백 `APPCHECK_ENFORCE=false`). |
...
| 커밋 | 내용 |
|---|---|
| (v117 / 워커 r13) | **랭킹 위조 '문턱 높이기' — 서버 세션 채점 도입 (트리비얼 콘솔 위조 무력화 · 정직한 300/일 수용) [코드 완료·배포 대기]** — 배경: v110/r6 att 상한은 원샷 위조만 막고 '매일 콘솔로 doneByDay 를 조금씩 심으면 상한까지' 잔존했고, 상한을 정직한 헤비학습자(하루 ~300단어)에 맞춰 올리면 위조창도 커지는 근본 긴장이 있었다. App Check 만으론 부족(앱 인스턴스만 증명 → 진짜 페이지의 콘솔 위조는 토큰 재사용으로 통과). **해법(사용자 선택 '서버 세션 채점')**: 랭킹 크레딧을 **서버가 발급한 퀴즈 세션을 통과한 단어**에만 준다 → 콘솔로 `doneByDay` 를 통째로 심는 '한 줄 위조'는 무력화. **★정직한 한계(독립검증에서 확인)**: 단어 id=영어 단어이고 세션 시작 때 클라가 id 를 알려주므로 **id-에코(답=id)로 팩 검증 통과 가능** → 결정적 스크립터는 세션 흐름을 스크립트해 상한까지 위조 가능(**완전 차단 아님 · C1 근본 한계**). 얻는 것: 트리비얼 콘솔 치팅 사망 + App Check(앱 밖 스크립트 차단) + rate-limit + 상한 바운드. '정직한 300 vs 가짜 300' 구분은 여전히 불가(상한이 둘 다 묶음). CLAUDE.md 4단계 전과정: **설계 독립검증 87 · 스펙 86 · 플랜(4지적 반영) 통과**, 구현은 컴포넌트별 검증. **구성**: ① `scripts/build-pack-answers.mjs`(신규) — 단어파일에서 id→{he=sha256(SALT|en|스펠링), hk=sha256(SALT|ko|뜻 전체)} 번들 생성, **트윈 두 워커 마커에 인라인 주입**(SW 단일파일 배포·import 불가 대응, ~1131 id). ② 워커 트윈 `/quiz/start`(due∩실재 확인·팩/개인 분류·HMAC 1회성 세션)·`/quiz/submit`(세션 소비·rate-limit·정답대조 `packAnswerOk` he‖hk 모드무관·`ver_pack`/`ver_pers` (id×날짜) 적립·버스트). App Check **JWKS**(RS256·`aud=projects/PROJECT_NUMBER`·enforce/모니터링). REV r12→**r13**. ③ 점수 **하이브리드**: `packWk=min(팩유니크,PACK_WEEK_CAP=2000)+personalWk=min(개인유니크,PERSONAL_WEEK_CAP=500)`. 팩=서버 정답검증 → 사실상 무제한(커리큘럼 ~1399 자연상한), 개인/OCR=검증불가 → 상한. 일 버스트 `PACK_DAILY_BURST=350`(정직 300 수용·이번주 신규만). ④ 마이그레이션: `combinedWk = ATT_RETIRED? verWk : max(attWk, verWk)` — handleSync·handleBoard 통합(회귀0: ver 비면 attWk 그대로·att 은퇴 시 완전 ver). ⑤ 클라 `cloud.js`(App Check init·격리 import·`quizStart`/`quizSubmit`)·`index.html`(전투 정답→버퍼→배치 quizStart/submit; 소탕·군집매치는 답 미생산이라 제외). `firebase-config.js` APPCHECK_SITE_KEY(빈값=off)·PROJECT_NUMBER(=363343730753). **정직한 한계(C1)**: 정답이 클라 평문이라 '정답 아는 봇 자동풀이(T3)'는 완전차단 불가 — App Check+버스트로 억제(T1 트리비얼 위조가 죽은 게 큰 성과). 오프라인/미로그인 학습 불변(회귀0). 검증: `node --check` 전 파일·빌드스크립트 라운드트립 3/3·C2/C3 유닛 22/22·**세션흐름 기능테스트 18/18**(발급·분류·미실재탈락·정답대조/오답미집계·리플레이거부·세션밖무시·combinedWk·retired·App Check)·전 함수 트윈 바이트 동일. 캐시 sw.js v116→**v117**·index BUILD v117. `firestore.rules` **무변경**. **⚠️ 적용(사용자 작업 필수 — 코드로 못 끝냄)**: (1) 워커 env `QUIZ_SECRET`·`ANSWER_SALT`(비밀·생성) + `PROJECT_NUMBER`=363343730753 + 캡값 설정, `ANSWER_SALT="…" node scripts/build-pack-answers.mjs` 실행(번들 채움) → **트윈 재배포**(`v:"r13"` 확인). (2) Firebase App Check 콘솔 등록·reCAPTCHA v3 사이트키 → `firebase-config.js` 기입·배포 → 모니터링 후 Firestore enforce. **상세: `docs/server-scoring.md`·`docs/appcheck-setup.md`.** ★배포 전엔 `/quiz/*` 미사용(클라가 404 폴백)이라 기존 att 랭킹 그대로 동작(무중단). |
| (v116 / 워커 r12) | **학생의 '연속일수(🔥)' 자기보고 위조 차단 — 서버 관측(att)만으로 산정 (워커 r12 + 앱)** — 확인(독립검증 CONFIRMED): 단어값(주간 완료수)은 서버 원장 `att` 상한(v110/r6)으로 원샷 위조가 막혔지만, **연속일수는 여전히 학생이 자기 Firestore 문서에 값을 써서 임의로 조작 가능**했다. 원인: v111/v115 가 '정직한데 서버 미관측 학생 false-0' 를 없애려고 연속을 **`max(서버 att, 자기보고)`** 로 산정 → 자기보고(대시보드 `summary.streak`, 워커 `private/state.meta.streak`)가 그대로 표시·랭킹에 반영. `firestore.rules` 는 `summary.streak`(validSummary 타입만)·`private/state`(필드 검증 0)에 **값 상한이 없어** 콘솔로 `streak=99999`+`lastDay=오늘` 쓰면 대시보드·인게임 랭킹에 🔥99999(✓만 없음). 자기보고 주입 3+1 지점: 대시보드 `card()` `max(srvStreak,selfStreak,mStreak)`, 워커 `handleSync`/`handleBoard` 내 행 `unifiedStreak(att,self)`, 인게임 '내 행' 클라 `max(e.streak,liveStreak())`. **수정(att-only 복원 = r10 신뢰모델)**: ① 워커 쌍둥이(worker.js·worker-dashboard.js) `handleSync`·`handleBoard` 내 행 연속을 `streakFromDays(att)`(서버 오늘키만·백데이트 불가)로 되돌림 — 자기보고 미포함. att 읽기 실패 시에만 서버관측 `st:` 카운터 폴백(학생이 못 쓰는 값). `unifiedStreak`/`parseSelfReport` 는 미사용 처리(삭제 않고 '호출부 부활 금지' 주석). REV r11→**r12**. ② 대시보드 `card()`: 연속·학습일 '숫자'와 ✓ 를 **서버 att 값만** 사용(`verifiedOn` 분기). ★핵심: 워커 응답 O 인데 att 없는 학생(=미동기화 치터)은 **헤드라인 0** → 콘솔로 `summary.streak` 만 위조해도 헤드라인 숫자를 못 만든다. 자기보고가 서버값보다 크면 **'· 앱 N일 미검증'** 라벨 힌트로만 노출(0~99999 클램프·숫자/순위 미반영) → 정직한 미관측 학생도 투명하게 보이고 위조도 드러남. ✓ 는 `srv>0` 일 때만(0에 안 붙임). 워커 OFF(`vb=null`)면 자기보고 폴백(기존 '⚠️ 미적용' 배너). ③ 인게임 랭킹 '내 행' 클라 `liveStreak()` max 주입 제거 → 서버 att 값 사용(서버가 오늘 live att 보정으로 '방금 푼 오늘분' 반영하므로 무손실). 안내 배너 문구 att-only 로 정정. **단어값(wk)·att 상한·`firestore.rules` 무변경**(위조 차단은 표시·집계 계층에서 — 단어값과 동일 전략). **트레이드오프(의도)**: 연속 숫자가 서버 att 만 반영 → att 가 못 본 정직한 학습분은 헤드라인에서 빠지고 '미검증 힌트'로 노출(v111 이 없애려던 false-0 를 '숨기지 않고 투명하게'로 재균형). 순위 주지표는 wk(단어)라 연속은 tiebreak·표시에만 영향. **레거시**: 기존 r11 보드 KV(부풀린 `meta.s`)는 학생 다음 /sync 때 att-only 값으로 자가치유(`boardWriteDue` 가 s 변경 감지). 검증: CLAUDE.md 4단계 — Plan 독립검수 **91/100 통과**. 자동검증 `node --check`(워커2·words·index 메인)·트윈 로직 파리티(변경 라인 동일·잔여 diff 는 기존 let/var 스타일+footer 주석)·streakFromDays/anchorDay att-only 순수로직 **6/6+불변식**·card() 게이팅 **7/7**(미동기화 치터·위조 헤드라인=서버값). 캐시 sw.js v115→**v116**·index.html BUILD v115→v116. `firestore.rules` **무변경**. **⚠️ 적용: rank-worker 재배포 필요**(worker.js 또는 서비스워커 형식이면 worker-dashboard.js 붙여넣기·Deploy — 확인은 워커주소 열어 응답 `v:"r12"`) **+ 앱(Pages) 배포**. |
| (v115 / 워커 r11) | **인게임 랭킹 '연속(🔥)'을 대시보드 값과 완전 통일 (양방향 불일치 해소)** — 증상: 연속학습일이 **선생님 대시보드와 인게임 랭킹에서 다른 값**(학생 승부욕 유도용 지표라 일치 필요 · 사용자 요구는 "랭킹을 대시보드 값으로"). 원인: 같은 🔥인데 두 화면이 **다른 산식**. 대시보드 `card()`(index.html:3688~3693)는 `max(서버 att 걷기, 살아있는 자기보고 스칼라, att∪자기보고 히트맵 합집합 걷기)` — 자기보고 포함. r10 랭킹은 보드 KV `meta.s = /sync 시점 streakFromDays(att)` **만** — att 가 못 본 정직한 연속은 랭킹에서 누락(정방향 불일치), 반대로 학생이 sync 를 멈추면 보드값이 얼어붙어 **끊긴 연속이 랭킹에 잔존**(역방향 불일치). **수정(워커 r11 · worker.js·worker-dashboard.js 쌍둥이)**: ① `getState` 가 상태문서에서 자기보고 원천(`meta.streak/lastDay/dailyHistory` → `parseSelfReport`, 35일 창·정수 안전파싱·0~99999 클램프)을 추가 파싱(mask 없이 읽던 문서라 **추가 read 0**). ② `unifiedStreak(att, self, today)` 가 **대시보드와 동일한 max 산식**으로 연속 산정 → `/sync` 보드 기록·`/board` 남의 행·내 행 모두 대시보드 🔥와 같은 숫자. ③ 보드 `meta` 에 감쇠 anchor `d`(활동일·자기보고 lastDay 중 오늘/어제) 저장, `/board` 가 **정렬 이전에** `d` 로 감쇠(오늘/어제 아니면 0) → 역방향 잔존 제거. 프리즈(자기보고 lastDay=어제, 히트맵 없음) 학생도 anchor 확보. r10 레거시(d 없음)는 `at`(마지막 기록) 근사 감쇠 후 다음 sync 에 정밀 치유. `boardWriteDue` 에 `d` 변경 비교 추가(같은 s·w 라도 anchor 전이 시 기록·r10→r11 이관 트리거). ④ 폴백 경로(워커 OFF · `cloud.js getRank/getGlobalRank`)도 `days`(오늘/어제 활동)로 **읽기 시점 감쇠** — leaderboards 쓰기는 규칙으로 잠겨(2026-07-16) 이 경로 항목은 동결 레거시라 읽기 감쇠가 유일한 정합 수단. **트레이드오프(의도)**: 랭킹 연속이 자기보고를 포함(대시보드가 이미 ✓ 없이 표시하던 것과 동일 신뢰수준 · 스칼라 0~99999 클램프·합집합 35~40일 자연상한). **주 순위 지표 wk 는 여전히 att 상한(위조불가)** — 연속은 동점 tiebreak·표시에만 영향. 검증: 순수로직 **39/39**(fsNum·parseSelfReport 클램프/창/정수파싱·unifiedStreak≡대시보드 max·anchorDay 프리즈·불변식 streak>0⇒anchor≠null·staleByAt·boardWriteDue d감지·감쇠 시뮬) + **쌍둥이 파리티**(worker-dashboard.js 를 vm 로 로드해 동일 벡터 동작 일치) + cloud 폴백 감쇠 5/5 + `node --check` 4파일. 캐시 sw.js v114→**v115**·index.html BUILD v114→v115. `firestore.rules` **무변경**. **⚠️ 적용: rank-worker 재배포 필요**(Service Worker 형식이면 `worker-dashboard.js` 붙여넣기·Deploy — 확인은 워커주소 열어 응답 `v:"r11"`) **+ 앱(Pages) 배포**. 배포 직후 각 학생은 다음 /sync 부터 unified 값·anchor 로 갱신(자연 치유). |
| (v112 / 워커 r9) | **대시보드 '반 랭킹'과 인게임 '랭킹 창'을 '단일 소스'로 완전 통일 (false-0 포함)** — r8(스로틀 완화) 후에도 두 화면 숫자가 달랐다(실례: 이성현 랭킹창 105 / 대시보드 0). 원인: 두 화면이 '이번 주 완료 단어'를 **각자 계산** — 인게임은 워커 보드값(서버 `countWeekDone`), 대시보드는 앱이 자체 재계산(`weekWordsOf`: 자기보고 `summary.weekWords` 를 att 로 상한). **서버 출석원장(att)이 빈 학생은 대시보드가 `min(raw, att상한=0)=0` 으로 깎아 false-0**(연속일수 0 버그와 동일 뿌리), 반면 인게임 보드값은 실제 완료수(105)를 보였다. **수정(단일 소스)**: 워커 `/teacher` 가 학생별 **반 보드 KV(`c:주:반:uid`)의 점수 w**(=`/board` 가 인게임에 쓰는 바로 그 값)를 `weekWords` 로 실어 보내고, 대시보드 `weekWordsOf` 가 **자체 att-상한 재계산을 버리고 그 값을 그대로 표시** → 두 화면이 **같은 KV 키를 읽어 구조적으로 100% 일치**, false-0 소멸. 보드값 없으면(구버전 워커·0단어·미동기화) 기존 계산 폴백(무회귀). `cloud.js teacherBoard` 가 `weekWords` 파싱, `index.html weekWordsOf` 가 우선 사용. **위조 방지**: 표시 기준을 리더보드와 하나로 통일(리더보드가 쓰던 값 그대로 · 별도 완화 없음). 검증: `node --check`(워커2·cloud·index 메인)·쌍둥이 파리티·`weekWordsOf` sim 6/6(이성현 105·보드값·폴백·워커OFF·0단어). 워커 REV r8→r9. 캐시 v111→v112. **⚠️ 적용: rank-worker 재배포(`v:"r9"`) + 앱(Pages) 배포.** ★참고: 인게임 '내 행'만 실시간 재계산이라 '보는 본인'은 대시보드 저장값과 순간 다를 수 있음(다른 학생 행은 동일). 0단어 학생은 인게임 숨김(>0)·대시보드 0 표시(집합 차이, 별개). |
| (워커 r8) | **대시보드 '반 랭킹' vs 인게임 '랭킹 창' 이번주 완료단어 수 불일치 통일** — 둘 다 '이번 주(월~오늘) 완료 단어(서버 att 상한)'로 **매주 월요일 리셋**되지만 **신선도**가 달랐다. 대시보드 `weekWordsOf` 는 각 학생 마지막 동기화값(자기보고 유니크를 att 로 상한)을 바로 표시해 신선한데, 인게임 랭킹 창(`renderClassRank`→워커 `/board`)은 **내 행만 실시간·다른 학생 행은 KV 스로틀 저장**(`boardWriteDue`: RANK_STEP 기본 5단어/RANK_FLUSH_MIN 15분)이라 최대 5단어·15분 지연 → 대시보드보다 뒤처져 보였다(무료 KV 쓰기 한도 보호 스로틀의 부작용). **수정(워커 쌍둥이만)**: `handleSync` 의 `step` 기본값 **5→1** → 점수가 바뀌면 매 sync 기록(dw=0 이면 여전히 미기록=불필요 쓰기 방지). 이제 인게임 다른 학생 행도 '각 학생 마지막 동기화값'을 반영 → **두 화면 일치**. **위조 방지(att 상한 `weekLedgerBound`)·집계 로직 무변경** — 기록 빈도만 완화(Paid KV 라 안전, env `RANK_STEP` 재상향 가능). 검증: `node --check`·쌍둥이 파리티·boardWriteDue sim 9/9(step5 지연 재현·step1 즉시기록·dw=0 미기록·att-cap 불변). 워커 `REV r7→r8`. **앱 무변경**(렌더 그대로 — 워커가 더 신선한 값을 줄 뿐). **⚠️ 적용: rank-worker 재배포만**(대기 중인 r7 재배포에 포함 → 재배포하면 r7+r8 함께 · `v:"r8"` 확인). 참고: 인게임 '우리 반' 탭은 '보는 사람 소속 반', 대시보드는 '선택한 반' 기준이라 여러 반 관리 선생님은 스코프가 다를 수 있음(별개 사안). |
| (v111) | **선생님 대시보드 '연속학습 일수'가 일부 정직한 학생에게 0으로 뜨는 문제 근본 수정 (앱 v111 + 워커 r7)** — 증상: 공부하는데도 대시보드 🔥연속이 0인 학생 몇 명(선생님 출석 확인용 지표라 누락 불가). 원인(독립검증): 대시보드 연속·학습일이 **서버 출석원장 `att` 에서만** 산정(`streakFromDays`)되는데, `att[날짜]` 는 '학생 로그인 + **그 KST일에** 워커 `/sync` 성공 + 서버가 읽은 `doneByDay` 유효완료>0' 일 때만 기록되고 **과거 소급 불가**(위조 방지 핵심). 이 사슬이 끊긴 날(오프라인 학습·디바운스 전 종료·인앱 hang·**기기 시간대≠KST**·빈 words 필터)은 원장에 영구 결손 → `verifiedHere?int(v.streak):…`(index.html:3669) 이 서버 0 을 그대로 표시(자기보고 폴백 없음). 학생 앱은 자체 `liveStreak()` 로 멀쩡히 보이나 대시보드만 0. **핵심 긴장**: '연속 위조 100% 불가'와 '정직한 학생 100% 누락 없음'은 동시 불가(서버는 못 본 날을 검증 못 함) → 사용자 선택 **둘 다**(표시는 누락 방지, ✓ 검증은 서버값 유지). **수정**: ① **Part A(표시 union·클라)** — `renderTeacherStudents` 연속·학습일 = `Math.max(서버 관측, 자기보고)`, ✓ 는 `srv>0 && srv>=self`(서버가 표시값 이상 관측) 일 때만 → 정직한데 서버 미관측인 학생 **false-0 제거**(오늘학습·히트맵과 동일 union 철학, v104), 위조(self 부풀림)는 ✓ 없이 떠 선생님이 구분. 지표별 `chkStreak`/`chkStudy` 분리·배너 문구 보정. `weekWordsOf`(반 랭킹) 무변경(위조방지 서버캡 유지). ② **B1(KST 날짜 고정)** — 앱 `today()`/`addDays()`(index.html)·`todayStr()`/`addDaysStr()`(cloud.js)를 **KST 고정**(`Date.now()+9h`→getUTC*, addDays 는 `Date.UTC` 순수 달력가산 = 워커 `kstToday`/`isoAddDays` 와 동일). 앱·cloud·워커 3자 KST 정렬 → att 조회·doneByDay 키·`summary.weekOf`·`sm.lastDay↔교사 t` 비교 일치. **KST 기기(한국 학생 대다수)는 문자열 동일=회귀 0**, 비-KST 기기만 서버와 정렬(due 도 1회 자기교정 재정렬·무해). ③ **B2(빈 wordIds 하드제로 제거·워커 쌍둥이)** — `getState` 가 words 맵 비면(리셋직후·부분쓰기) 완료를 전부 탈락시키던 비대칭을 all-pass(null)로 승격(읽기실패와 동일 가용성). 정상 학생 무변경, 위조상한은 att 캡이라 무해. ④ **B3(att 유실 관측성·워커 쌍둥이)** — `recordAttendance` 가 '그날 첫 출석' write 실패 시 `degraded=true`, `/sync` 응답에 `attDegraded` 필드(재시도는 안 함=쓰기증폭 회피, 다음 sync 자기치유). 워커 `REV r6→r7`. **잔여(불가역)**: 한 번도 sync 안 한 완전 로그아웃 학생은 Firestore summary 자체가 없어 대시보드에 안 뜸 → 지렛대는 로그인 유도(v107 `renderSyncWarn`), 서버 견고화 범위 밖. 검증: 순수로직 **93/93**(앱 날짜==워커 kstToday/isoAddDays·**KST 기기 no-op 회귀0**·Part A 매트릭스 false-0제거·✓정직·단조비감소·빈words all-pass·streakFromDays 갭·degraded) + `node --check`(워커2·cloud.js·index.html 메인블록) + 쌍둥이 파리티(변경 3함수 마커 1:1) + Playwright 부팅 스모크 **9/9**(브라우저 today()=KST·liveStreak 정상·홈 렌더·JS예외0). CLAUDE.md 4단계: Plan 독립검수 **78→90 통과**(z 블로커·구조적잔여 반영), 구현 독립검수 진행. 캐시 sw.js v110→**v111**·index.html BUILD v102→v111(정합). `firestore.rules` **무변경**. **⚠️ 적용: 앱(Pages) 배포**(Part A + B1 즉시 적용) **+ rank-worker 재배포**(B2/B3·`v:"r8"` 확인). Part A 만으로 대시보드 누락 즉시 해소(Pages), 워커 재배포는 ✓ 커버리지·비-KST 정합 강화. |

### 이번 세션(2026-07-16) 추가 — 랭킹 미등재(KV 쓰기 한도) 근본 대응

| 커밋 | 내용 |
|---|---|
| (v110) | **단어수(주간 랭킹) 조작 근본 차단 — 서버 관측 원장(`att`) 상한 (워커 r6 + 앱)** — 증상: 학생이 단어수를 위조해 랭킹 1위(재발). 원인: 점수는 워커만 KV에 쓰지만(leaderboards 잠금), 워커가 세는 원천 `users/{uid}/private/state` 의 `doneByDay`·`words` 를 **둘 다 학생이 자유쓰기**(학습저장 필수라 못 잠금) → r5 "words 맵 대조" 필터가 **자기참조라 무력**(자기 `words` 맵에 가짜 단어를 넣고 `doneByDay` 에 그대로 심으면 통과 → 임의 N 점수). **+ 2번째 경로**: 선생님 대시보드 '반 랭킹'(`weekWordsOf`)이 자기보고 `summary.weekWords` 로 순위 → 콘솔로 그 값만 Firestore에 직접 써도 1위(워커 무관). **수정**: 워커가 **서버 시계로만 기록하는 백데이트 불가 원장 `att:{uid}`** 로 주간 점수에 상한 — `점수 = min( 스냅샷 유니크수, Σ_{월~오늘} min(att[그날], RANK_DAILY_CAP=60) )`(총합 `RANK_WEEK_CAP=300` 컷). `att` 는 '서버 오늘' 키에만 기록돼 소급 위조 불가 → `doneByDay` 통째 위조도 "서버가 실제로 며칠에 걸쳐 관측한 만큼"으로 묶임(원샷 99999→일캡=즉시 1위 불가). ① `worker.js`·`worker-dashboard.js`(쌍둥이): handleSync `wk=min(rawWk,bound)`, handleBoard '내 행'도 동일 상한 + **무조건 교체**(옛 비캡 위조 KV 값 자가정정), `recordAttendance` 는 `{map,ok}` 반환(att 읽기 throw=인프라장애면 **무클립**=가용성 우선, 클라가 서버 KV 읽기를 못 건드림), `/teacher`·`/sync` 응답에 caps 실음, `weekLedgerBound`/`capOf` 순수함수(타입·음수·소수·주범위 방어), REV r5→**r6**. ② `cloud.js teacherBoard` 가 caps 통과. ③ `index.html weekWordsOf` 가 **워커 배포(`vb`) 시 항상** att 상한 적용 → **att 흔적 없는 치터**(=`summary.weekWords` 만 위조·`/sync` 미호출)는 **0**(서버 미관측=미집계, 리더보드 철학과 동일); 워커 OFF(`vb=null`)면 전원 raw 폴백(무회귀·"⚠️ 서버 검증 미적용" 배너). 정직 학생은 스냅샷이 상한 이하라 **점수 불변**(요일간 복습·배틀종료 sync 로 헤드룸 큼). `firestore.rules` **무변경**(leaderboards 쓰기잠금 견고·state 쓰기권한은 학습저장 필수). 캡은 env `RANK_DAILY_CAP`/`RANK_WEEK_CAP` 로 **재배포 없이** 튜닝(정직 클리핑 시 상향·치팅 잔존 시 하향). 검증: `weekLedgerBound`/`capOf` 순수로직 **17/17**(정직불변·원샷→60·분산→과거실관측만·매일→300=주캡·주경계·미래/음수/문자/소수 방어·커스텀캡) + `node -c` 워커2·`cloud.js`·index.html 메인블록 통과. CLAUDE.md 4단계 독립검수: **Plan 83→88 통과**(경로2·위조KV 정리 누락 반영), **구현 82→90 통과**(경로2 att-없는 치터 우회 C1 수정). 캐시 v109→**v110**. **⚠️ 적용: rank-worker 재배포 필요**(worker.js 또는 서비스워커 형식이면 worker-dashboard.js 붙여넣기·Deploy — 배포확인은 워커주소 열어 응답 `v:"r6"`) **+ 앱(Pages) 배포**. 기존 위조 KV 항목은 다음 sync 자가치유되거나 콘솔에서 `g:<주월>:<uid>`·`c:<주월>:<반ID>:<uid>` 수동삭제(`rank-worker/README.md`). |
| (v109) | **선생님 대시보드 '이번 주 완료 단어 수'가 인게임 랭킹과 불일치 수정 (대시보드 9 vs 랭킹 20)** — 원인: 두 화면이 '이번 주 완료 단어'를 **서로 다른 원천·방식**으로 집계. 인게임 랭킹(워커 배포됨)은 서버 `countWeekDone(doneByDay)` = **주간 유니크**(정확). 대시보드 `renderTeacherStudents.weekWordsOf` 는 서버 `att`(KV 쓰기 절약 스로틀로 실제보다 **undercount**) ∪ `summary.daily` 의 **날짜별 합**을 쓰는데, `mergedDaily`(index.html:3655)가 undercount 된 `att` 로 정확한 자기보고(`summary.daily.w`)를 덮어써 랭킹보다 작게 떴다(작은 단위로 학습한 학생일수록 심함). 수학적으로 데이터가 완전하면 날짜별 합 ≥ 주간 유니크라, **대시보드<랭킹**은 대시보드 원천이 손실됐다는 증거. **수정(클라 전용 · 워커/집계 로직 무변경)**: ① `cloud.js writeSummaryDoc` 가 `summary.weekWords`(이번 주 완료 **유효단어 유니크** — `collectWeekIds` + 현재 단어목록 필터 → 워커 `countWeekDone` 과 **동일 계산**)와 `summary.weekOf`(이번 주 월요일)를 요약에 기록. ② `weekWordsOf` 가 **인게임 랭킹 원천에 맞춰** 집계: 워커 ON 이면 `weekWords`(유니크, `weekOf` 일치 시)로 → 인게임과 일치; 워커 OFF(폴백)면 인게임이 `weekSum`(날짜별 합)이므로 대시보드도 날짜별 합 유지 → **두 구성 모두 대시보드=인게임**. 구버전 앱·미동기화·지난주 값은 날짜별 합으로 폴백(무회귀). ③ `firestore.rules validSummary` 에 `weekWords`(number)·`weekOf`(string) 타입가드 추가(심층방어 — 대시보드는 어차피 `int()` 강제라 규칙 미배포여도 안전). ★`weekWords` 는 **자기보고값**(정답률·포획과 동일 신뢰수준, 서버 검증 아님) — 인게임 랭킹(서버 집계)은 그대로 위조 불가. 검증: `node --check`(cloud.js + 인라인 4블록) + 로직 시뮬 16/16(유니크·유효필터·`weekOf` 가드·0값 존중·구버전 폴백·워커 ON/OFF 양쪽 대시보드=인게임·대소문자 dedup, OLD=9→NEW=20 재현). CLAUDE.md 4단계: 독립검수 Plan 88 / 구현 88 통과. 캐시 v108→v109. **앱(Pages)만 배포하면 적용**(워커 무변경 · 규칙 타입가드는 콘솔 반영 시 심층방어 강화, 필수 아님). |
...
> **★ 현행(v110/r6) 요약 — 이 블록을 우선한다**: 주간 점수 = `min( 스냅샷 유니크수, Σ_{월~오늘} min(att[그날], RANK_DAILY_CAP) )`(총합 `RANK_WEEK_CAP` 로 컷). 서버가 서버 시계로만 관측하는 **백데이트 불가 원장 `att`** 로 상한을 씌운다 → `doneByDay`·`words` 를 자기 문서에 통째로 위조해도 "서버가 실제로 며칠에 걸쳐 관측한 만큼"으로 묶인다(원샷 99999 = 즉시 무력화). **리더보드(워커 `/board`)와 선생님 대시보드 '반 랭킹'(앱 `weekWordsOf`) 두 경로 모두** 같은 상한 적용(대시보드는 `/teacher` 로 받은 `att` 로 자기보고 `summary.weekWords` 를 컷). ★워커 배포 시 대시보드는 **att 흔적이 없는 학생**(워커에 한 번도 동기화 안 함 = `summary.weekWords` 만 콘솔 위조한 치터)을 **0** 으로 표시(서버 미관측=미집계). 워커 미배포(`vb=null`)면 전원 자기보고 폴백(무회귀). 아래 원래 서술 중 '배포단어 교집합·천장=배정 단어수'는 **v100 이전 설계**라 현행과 다르다(현행은 개인·기본팩·배포 단어 모두 카운트 후 att 상한). 상세: `rank-worker/README.md`.
...
- **한계(v110/r6 갱신)**: 콘솔 임의값·원샷·과거소급 위조는 `att` 원장 상한으로 **차단**(원샷 99999 → 일 캡). 완전 0% 는 아님 — 매일 실제 접속·동기화하며 매일 캡만큼 위조하면 주 캡까지 가능(**즉시 1위 불가**·여러 실제 날 필요·`att` 타임스탬프 감사 가능한 강제 한계). 캡은 `RANK_DAILY_CAP`(기본 60)·`RANK_WEEK_CAP`(기본 300) 로 재배포 없이 튜닝.
