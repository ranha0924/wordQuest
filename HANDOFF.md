...
> - **선생님 권한 보안(2026-07-15)**: 과거 하드코딩 비번(`TEACHER_PW='8889'`) 소프트 게이트는 소스 열람·콘솔(`Cloud.chooseRole('teacher')`)로 우회됐고, 실제로 한 학생이 스스로 선생님이 됨. → **서버 강제로 전환**: 선생님 승격을 위조 불가 커스텀 클레임(`request.auth.token.teacher`)으로만 인정하고, 클레임은 **`teacher-worker/`(Cloudflare Worker)가 비밀번호(Secret `TEACHER_PW`)를 서버에서 확인한 뒤 Firebase Admin 으로만** 부여. `firestore.rules`가 `role='teacher'`·반 생성/수정/코드·`classPacks` 쓰기를 `isTeacher()`(클레임)/마스터만 허용. 비번은 앱·소스에 없음. 셋업·배포순서·재성 계정 정리는 **`docs/teacher-auth-setup.md`**. 앱 연결값: `firebase-config.js` 의 `window.TEACHER_AUTH_URL`.
