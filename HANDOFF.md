...
- **영입소** 등록 경로: (1) 붙여넣기(`abandon - 버리다` 형식, `parseText()`), (2) 샘플 20개 버튼, (3) **"고1 필수 어휘 팩 담기"**(`packbtn`, 추천·골드 실선) = 주제별 큐레이션 224개, (4) **"고빈도 영단어(범용) 담기"**(`bankbtn`, 이전 "교과서 고빈도"에서 **정직하게 개칭**) = 범용 은행. (3)·(4) 모두 **하루 25개씩 순차 출현**(staggered due)·이미 담긴 단어 제외.
